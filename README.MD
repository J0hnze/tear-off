...
Copy the following ` to `.env` and update where needed:
```env
TICKETS_USER=admin
TICKETS_PASS=changeme
NO_PRINTER=false
TICKETS_PRINTER_NAME=BIXOLON SRP-E300 # Exact Windows printer queue name
TICKETS_HOST=0.0.0.0
TICKETS_PORT=5000
TICKETS_DB=tickets.db
TICKETS_SECRET=change-me-secret
TICKETS_PRINT_COLS=46
TICKETS_DEFAULT_TAGS=work,personal
TICKETS_THEME=dark
```
...
python app.py
```
*if you change the NO_PRINTER value to true it will just print to the console, i have plans to 'print' the tickets to a text message or tg message but thats for another day*
...
Click "✓ Mark done" button to complete a ticket.
...
- ✨ **UUID-based ticket IDs** - Replaced integer IDs with UUIDs for better uniqueness
- 🖨️ **Print weekly button** - Added "Print All" button to Weekly view
- ✓ **Mark done functionality** - Implemented ticket completion workflow
- 🔧 **Improved printer handling** - Fixed buffer flushing for reliable thermal printing
...
- **macOS/Linux** - Web UI works, printer requires manual setup with escpos library * needs testing more
...
- [ ] Export Calendar / subscription
  - [ ] apple calendar
  - [ ] google calendar
- [ ] SMS/text App notifcations
  - [ ] SMS integration
  - [ ] Signal Integration
  - [ ] Telegram integration
  - [ ] WhatsApp integration
  - [ ] Send “Today at 09:00” summary
...
## WIBNI
- [ ] A read-only URL protected by Basic Auth
